...
class HuggingFaceProvider(Provider):
    ...
    def _ping(self) -> None:
        api_key = os.environ.get("HF_TOKEN") or os.environ.get("HUGGINGFACE_API_KEY")
        if not api_key:
            raise ProviderError("HF_TOKEN or HUGGINGFACE_API_KEY not set")
        try:
            import openai
            client = openai.OpenAI(api_key=api_key, base_url="https://api-inference.huggingface.co/v1")
            client.models.list()
        except Exception as err:
            raise ProviderError(f"Hugging Face connectivity check failed: {err}") from err
